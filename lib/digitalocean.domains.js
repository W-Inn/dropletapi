...
function makeRequest(fn, uri, options, callback) {
    ...
    fn(uri, options).once('complete', handler);
}
...
function makeRequestJson(fn, uri, data, options, callback) {
    ...
    fn(uri, data, options).once('complete', handler);
}
 ...
 *
 */
 Domains.prototype.addDomainRecord = function(domain_name, data, callback) {
     makeRequestJson(
         rest.postJson, this.baseUri+domain_name+'/records', data, {
             headers: {
                 'Content-Type': 'application/json',
                 'Authorization': 'Bearer ' + this.token
             }
